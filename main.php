  ...
  <div>
    ...
    <div class="post">
      <form action="" method="post">
        <p><?php echo $hello;?>、<?php echo htmlspecialchars($user['name'], ENT_QUOTES);?>さん<br>メッセージを投稿してください</p>
        <?php if(!empty($error['comment']) && $error['comment'] === 'blank'):?>
          <p class="error">*メッセージが書かれていません。</p>
        <?php endif;?>
        <textarea class="textare" name="comment" cols="65" rows="6"><?php if($comment !== ''){echo htmlspecialchars($comment,ENT_QUOTES);}?></textarea>
      ...
      <p><button id="reback" class="post_btn" onclick="location.href='main.php?id=<?php echo $id;?>'">返信をやめる</button></p>
    </div>
    <hr>
          ...
          <div class="top">
            ...
            <div class="side">
              <div class="side_top">
                ...
                <p>[<a onclick="clickBtn1();" href="main.php?id=<?php echo $id?>&res=<?php echo htmlspecialchars($post['id'],ENT_QUOTES)?>">返信</a>]</p>
                <?php if($_SESSION['id'] === $post['user_id']):?>
                  <p>[<a href="del.php?id=<?php echo $id?>&did=<?php echo htmlspecialchars($post['id'],ENT_QUOTES)?>"
                  style="color: #F33;">削除</a>]</p>
                <?php endif;?>
                <?php if($_SESSION['id'] === $post['user_id']):?>
                  <p>[<a id="del" href="edit.php?id=<?php echo $id?>&eid=<?php echo htmlspecialchars($post['id'],ENT_QUOTES)?>">編集</a>]</p>
                <?php endif;?>
              </div>
...
if(params.get('res')){
  document.getElementById("reback").style.display ="inline-block";
}else{
